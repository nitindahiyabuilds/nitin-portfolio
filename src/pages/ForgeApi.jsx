...
const sections = [
  {
    number: '01',
    title: 'The Problem',
    content: 'The purpose of Forge API was to build a backend where the engineering work goes beyond simply creating endpoints. The project brings API design, authentication, authorization, database design, migrations, testing, CI and containerized development into one practical system.',
  },
  {
    number: '03',
    title: 'Authentication',
    content: 'Forge API uses JWT-based authentication with the OAuth2 flow provided through FastAPI. Password hashing protects stored credentials before authenticated requests receive a token.',
  },
  {
    number: '04',
    title: 'Authorization',
  ...
  },
  {
    ...
    content: 'Forge API was developed with Git and GitHub using feature branches, meaningful commits, pull requests and CI checks as part of the normal engineering workflow.',
  },
];
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <p className="mt-5 max-w-2xl text-sm leading-relaxed text-white/85">
              Forge API is a backend system I built to go beyond basic CRUD and practice the engineering concerns that appear in real backend applications.
            </p>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {sections.map((section, index) => (
            ...
            >
              ...
              <p className="mt-4 text-sm leading-relaxed text-white/85">{section.content}</p>
            </motion.article>
          ))}
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/10 border border-white/10">
            {decisions.map((decision) => (
              <article key={decision.title} className="bg-black p-6 md:p-8">
                ...
                <p className="mt-3 text-sm leading-relaxed text-white/85">{decision.description}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="px-6 py-20 bg-black border-t border-white/5">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div>
            ...
            <p className="mt-6 max-w-xl text-sm leading-relaxed text-white/85">
          ...
          <div>
            ...
            <p className="text-white text-lg leading-relaxed">
              Forge API is an engineering project being used as a practical backend engineering build.
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="border-t border-white/10">
            {stackGroups.map(([category, technologies]) => (
              <div key={category} className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-3 border-b border-white/10 py-5">
                ...
                <p className="text-sm leading-relaxed text-white/85">{technologies}</p>
              </div>
            ))}
          </div>
        </div>
